...
export default function MenuItem({ food, type, add, onDelete, isSaved, handleSaveToggle, isMine, foodDelete, count }: MenuItemProps) {

    return (
        <div className="group flex items-center justify-between px-4 py-3 hover:bg-gray-800/50 transition-colors">
            <div className="flex flex-col gap-1 min-w-0">
                <div className="flex items-center gap-2">
                    <p className={`truncate ${type === 'add' ? 'text-sm font-semibold text-white' : 'text-sm text-gray-300'} flex gap-2 items-center`}>
                        <span>{food.name}</span>
                        {count && count > 1 && (
